...
use App\Entity\{User, Child, Subscription, SubscriptionPlan, Payment, SubscriptionHistory, StripeWebhookLog, FeatureAccess, AuthorizedContact, AuthorizedApp, EmergencyContact, EmergencyCall, GeoLocation, AuthToken, PasswordResetToken, UserLog, BanList, AdminLog, ParentalSettings, AdminConfig, AuditLog, BackOfficeStat};
use App\Enum\{SubscriptionStatus, SubscriptionEvent, PaymentStatus, EmergencyCallStatus};
...
class EntityFactory
{
    ...
    { $f = new FeatureAccess(); $f->setChild($c)->setFeature($feature)->setEnabled(true); return $f; }

    public static function authorizedContact(Child $c, int $i = 1): AuthorizedContact
    { $a = new AuthorizedContact(); $a->setChild($c)->setName('Contact'.$i)->setPhoneNumber('060000000'.$i)->setRelation('family'); return $a; }

    public static function authorizedApp(Child $c, int $i = 1): AuthorizedApp
    { $a = new AuthorizedApp(); $a->setChild($c)->setAppName('App'.$i)->setPackageName('pkg.app'.$i)->setIsAllowed(true); return $a; }

    public static function emergencyContact(Child $c, int $i = 1): EmergencyContact
    { $e = new EmergencyContact(); $e->setChild($c)->setName('Emerg'.$i)->setPhoneNumber('070000000'.$i); return $e; }

    public static function emergencyCall(Child $c, EmergencyCallStatus $status = EmergencyCallStatus::SUCCESS): EmergencyCall
    { $e = new EmergencyCall(); $e->setChild($c)->setStatus($status); return $e; }

    public static function geoLocation(Child $c, float $lat = 1.0, float $lng = 2.0): GeoLocation
    { $g = new GeoLocation(); $g->setChild($c)->setLatitude($lat)->setLongitude($lng); return $g; }

    public static function authToken(User $u, int $i = 1): AuthToken
    { $t = new AuthToken(); $t->setUser($u)->setToken('auth_'.$i)->setExpiresAt(new \DateTimeImmutable('+1 hour')); return $t; }

    public static function passwordResetToken(User $u, int $i = 1): PasswordResetToken
    { $t = new PasswordResetToken(); $t->setUser($u)->setToken('reset_'.$i)->setExpiresAt(new \DateTimeImmutable('+1 hour')); return $t; }

    public static function userLog(User $u, int $i = 1): UserLog
    { $l = new UserLog(); $l->setUser($u)->setIpAddress('127.0.0.' . $i)->setDevice('device'.$i)->setAction('LOGIN'); return $l; }

    public static function banList(User $u, int $i = 1): BanList
    { $b = new BanList(); $b->setUser($u)->setReason('Reason'.$i)->setBannedUntil(new \DateTimeImmutable('+1 day')); return $b; }

    public static function adminLog(User $admin, int $i = 1): AdminLog
    { $l = new AdminLog(); $l->setAdmin($admin)->setAction('ACT'.$i)->setTarget('Target'.$i); return $l; }

    public static function parentalSettings(User $u): ParentalSettings
    { $p = new ParentalSettings(); $p->setUser($u)->setPinCode('1234')->setSafeMode(true); return $p; }

    public static function adminConfig(int $i = 1): AdminConfig
    { $c = new AdminConfig(); $c->setKey('k'.$i)->setValue('v'.$i); return $c; }

    public static function auditLog(int $i = 1): AuditLog
    { $a = new AuditLog(); $a->setAction('ACTION'.$i)->setActor('actor'.$i); return $a; }

    public static function backOfficeStat(string $metric = 'users', float $value = 10): BackOfficeStat
    { $b = new BackOfficeStat(); $b->setMetric($metric)->setValue($value); return $b; }
}
